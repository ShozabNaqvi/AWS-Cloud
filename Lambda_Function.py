...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        if age is not None:
            age=int(age)
            if age > 65 or age<0:
                return build_validation_result(False, "age","You are not advised to make an investment",)

        ### YOUR DATA VALIDATION CODE ENDS HERE ###
        if investment_amount is not None:
            investment_amount=int(investment_amount)
            if investment_amount < 5000:
                return build_validation_result(False, "investment_amount", "Your initial investment is below the par level",)
